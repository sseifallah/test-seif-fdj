        ...
        DispatchQueue.main.async {
            if let tableView = self.tableView {
                ...
                if self.direction == .down {
                    ...
                    if self.maxResultsListHeight > 0 {
            ...
            for i in 0 ..< self.filterDataSource.count {
                ...
                if !self.inlineMode {
                    ...
                    if self.inlineMode, let filterAfter = self.startFilteringAfter {
                        if let suffixToFilter = textToFilter.components(separatedBy: filterAfter).last, (suffixToFilter != "" || self.startSuggestingImmediately == true), textToFilter != suffixToFilter {
...
public struct SearchTextFieldTheme {
    ...
    public static func lightTheme() -> SearchTextFieldTheme {
        return SearchTextFieldTheme(cellHeight: 30, bgColor: UIColor (red: 1, green: 1, blue: 1, alpha: 1), borderColor: UIColor (red: 0.9, green: 0.9, blue: 0.9, alpha: 1.0), separatorColor: UIColor.clear, font: UIFont.systemFont(ofSize: 13), fontColor: UIColor.black)
    }
    
    public static func darkTheme() -> SearchTextFieldTheme {
        return SearchTextFieldTheme(cellHeight: 30, bgColor: UIColor (red: 0.8, green: 0.8, blue: 0.8, alpha: 1), borderColor: UIColor (red: 0.7, green: 0.7, blue: 0.7, alpha: 1.0), separatorColor: UIColor.clear, font: UIFont.systemFont(ofSize: 13), fontColor: UIColor.white)
    }
}
